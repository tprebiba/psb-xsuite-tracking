...
p['qx_ini'] = 4.40
p['qy_ini'] = 4.45
p['correct_chroma'] = True # if True, chromaticity is corrected using XNOH0 (harmonic) sextupoles
p['chroma_plane'] = 'y' # to correct horizontal ('x') or vertical ('y') chromaticity
p['include_field_errors'] = True # if True, field errors are included
p['field_errors'] = {
...
# Setup acceleration
p['prepare_acceleration'] = 1 # 0: all RF OFF, 1: flat bottom (single RF at 8kV), 2: nominal PSB acceleration (double RF), 3: triple harmonic with acceleration
...
p['prepare_tune_ramp'] = 1 # if 1, 004_prepare_tune_ramp.py is executed
...
p['install_space_charge'] = True # if True, space charge is installed
